...
- When TaskNotes is installed, project tasks also carry a `projects` wikilink to their project note, so they show up in TaskNotes' project filter. Any existing project links on the task are kept, and the link follows the project note if it's renamed ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- When TaskNotes is installed, a TaskNotes task filed anywhere in the vault shows up in a project when its `projects` wikilink points at that project's note — it no longer has to live in the project's task folder. Editing such a task keeps its file where it is (it isn't moved into the task folder) while backfilling the project link ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- TaskNotes' `blockedBy` dependencies are read as task dependencies and shown on the Gantt. The full relation each entry encodes — its relation type and lag — survives editing the task here: removing a dependency drops its entry, and adding one records a finish-to-start relation with no lag, so a TaskNotes user loses no fidelity ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- When TaskNotes is installed, a dependency added here is also written as a TaskNotes `blockedBy` relation, so the blocker shows up in TaskNotes too — not only dependencies that originated there. The relation is written only with TaskNotes installed, so a plain vault never gains a stray `blockedBy` ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- When TaskNotes is installed, a subtask also carries a `projects` wikilink to its parent task note, so TaskNotes lists it under the parent's subtasks. The link follows the parent if it's renamed. Detaching is one-directional: clearing the parent here doesn't remove the link ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- When TaskNotes is installed, settings offer to align the two plugins' vocabulary in one click: adopt TaskNotes' status and priority lists, let TaskNotes read the field names this plugin already writes, and keep task titles in frontmatter so editing a task in TaskNotes no longer overwrites its title with the file name. Each alignment only changes settings — no task files are rewritten — and every one has a Revert that restores your previous values exactly ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- When TaskNotes is installed, a new "Sync time tracking with TaskNotes" setting adopts TaskNotes' time-entry sessions and minute-based estimates, so timers and estimates read and write the same shape in both plugins. Turning it on runs a one-time, guarded migration that converts existing per-day time logs into sessions and hour estimates into minutes; it asks for confirmation first, and left off, a co-installed TaskNotes task's minute estimate is no longer misread as hours ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
...
- A reload button in the project view's action bar re-reads the project from disk on demand — useful for pulling in TaskNotes tasks created or linked from outside the project's task folder without reopening the view ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
...
- After the one-time time-tracking migration to TaskNotes' shape, turning the "Sync time tracking with TaskNotes" toggle back off no longer misreads your minute-based estimates as hours (a 2-hour estimate no longer surfaces as 120 hours, and time sessions no longer disappear from the task editor). The time shape now follows the migrated files on disk, not the flippable toggle ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- With TaskNotes installed, editing a shared TaskNotes task that has a time estimate no longer multiplies that estimate 60× — its minute-based estimate is converted to hours when this plugin first takes over the note, instead of being reinterpreted as hours on the next read ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
- With TaskNotes installed, a due or start date you clear in TaskNotes stays cleared: clearing it (removing the field or writing `null`) no longer comes back the next time this plugin saves the task. A date-and-time value from TaskNotes is also adopted as a plain date rather than a raw timestamp ([#10](https://github.com/StepanKropachev/obsidian-pm/issues/10))
