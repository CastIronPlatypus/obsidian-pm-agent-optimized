...
import { DEFAULT_SETTINGS, makeProject, makeTask, timeShapeIsMinutes } from '../types'
...
/**
 * Pick a save path. New tasks get the bare slug. For an existing file the folder
 * may change (archive/unarchive), but the basename changes only when PM itself
 * named the file — renaming a file PM didn't name (e.g. a TaskNotes note called
 * "Fix the boiler") would sever the wikilinks TaskNotes and Obsidian backlinks
 * hang off its old basename. A file is PM-named when its basename is what PM's
 * current scheme produces (`taskSlug` is idempotent, so a PM slug is its own
 * fixed point) or a legacy scheme (`<slug>-<id8>`, or a 40-char title-slug cap).
 */
function resolveTaskPath(task: Task, folder: string, previousPath: string | undefined): string {
  ...
  // A basename PM's current scheme would produce is a `taskSlug` fixed point;
  // human- or TaskNotes-named notes (spaces, capitals) are not, so keep them.
  const pmNamed = taskSlug(previousBasename) === previousBasename
  if (previousFolder !== folder) {
    // Folder move (archive/unarchive): relocate, but re-slug only what PM named.
    return pmNamed ? desired : `${folder}/${previousBasename}.md`
  }
  // Same folder: legacy PM names are kept as-is to avoid churning existing files.
  ...
  return pmNamed ? desired : previousPath
}
...
export class ProjectStore implements TaskSource {
  ...
  /**
   * Whether this vault's task files are in TaskNotes' time shape (`timeEntries` +
   * minute estimates) on read/write. Keyed to the one-shot migration STAMP, not the
   * live `taskNotesTimeSync` toggle nor interop: the migration rewrote the files
   * durably and is one-way, so the shape survives the toggle flipping off or interop
   * being turned back off. Drives the `timeShapeMinutes` flag into hydrate/serialize.
   */
  private timeShapeMinutes(): boolean {
    return timeShapeIsMinutes(this.getSettings())
  }
  ...
  async loadTaskFile(
    ...
    try {
      ...
      const timeShapeMinutes = this.timeShapeMinutes()
      const interop = this.getSettings().taskNotesInterop
      const cached = this.app.metadataCache.getFileCache(file)?.frontmatter
      if (cached && this.isTaskFile(cached, tnConfig)) {
        const result = hydrateTaskFromFile(cached, '', file.path, external, timeShapeMinutes)
      ...
      const result = hydrateTaskFromFile(frontmatter, body, file.path, external, timeShapeMinutes)
      if (tnConfig) result.task.tags = stripMarkerTag(result.task.tags, tnConfig)
  ...
  private async saveTaskFile(
    ...
    try {
      ...
      if (kind === 'fm' && previousPath && !renamed) {
        ...
        if (existing instanceof TFile) {
          ...
          await this.app.fileManager.processFrontMatter(existing, (fm: Record<string, unknown>) => {
            ...
            task.foreign = collectForeign(fm, undefined, timeShapeMinutes)
            // This save stamps pm-task, so PM is claiming the file — convert any
            // TaskNotes-minutes estimate still sitting in foreign to our hours.
            adoptForeignTimeEstimate(task, timeShapeMinutes)
            this.reconcileBlockedByFromDisk(task, fm, project)
            this.reconcileSharedFieldsFromDisk(task, fm)
            const next = buildTaskFrontmatter(
              task,
              project,
              parentTask,
      ...
      // A rename: move the old file to the new path so Obsidian updates every
      // inbound wikilink (backlinks, TaskNotes `projects[]`/`blockedBy` uids)
      // rather than create+trash, which severs them until PM happens to re-save
      // the referrer. The moved file keeps its old body, so the in-place rewrite
      // below recovers an unhydrated description from it. importNoteAsTask uses
      // this same rename-then-rewrite pattern.
      ...
      if (target instanceof TFile) {
        // In-place full rewrite, atomic — covers a plain edit and the file we
        // just renamed. Recover description from disk when not hydrated.
        this.markSelfWrite(filePath)
        await this.app.vault.process(target, (content) => {
          ...
          task.foreign = collectForeign(frontmatter ?? {}, undefined, timeShapeMinutes)
          adoptForeignTimeEstimate(task, timeShapeMinutes)
        ...
        // Genuinely new file: no prior file on disk (or the rename source was gone).
